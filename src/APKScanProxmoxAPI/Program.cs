...
namespace APKScanProxmoxAPI
{
    public class Program
    {
        ...
        private static bool CreateNewSnapshot(string vmid, string name = "normal")
        {
            ...
            if (ExecuteCommand(ePVEAction.create, $"/nodes/fr1pve/qemu/{vmid}/snapshot/{name}/rollback") == null)
                return false;
            datum = DateTime.UtcNow; Console.WriteLine($"[{datum}] Restarting the VM {vmid}...");
            //second    reset the VM
            if (ExecuteCommand(ePVEAction.create, $"/nodes/fr1pve/qemu/{vmid}/status/reset") == null)
                return false;
            //wait for the reboot
            datum = DateTime.UtcNow; Console.WriteLine($"[{datum}] Restarting of {vmid} complete, waiting {config.vm_restart_time_in_seconds} seconds for boot...");
            Thread.Sleep(config.vm_restart_time_in_seconds*1000);
            //third     remove the normal snapshot
            datum = DateTime.UtcNow; Console.WriteLine($"[{datum}] Removing from {vmid} last good snapshot {name}...");
            if (ExecuteCommand(ePVEAction.delete, $"/nodes/fr1pve/qemu/{vmid}/snapshot/{name}") == null)
                return false;
            //fourth    create a new normal snapshot 
            datum = DateTime.UtcNow;  Console.WriteLine($"[{datum}] Creating a new snapshot from VM {vmid} with name {name}...");
            Dictionary<string, string> data = new Dictionary<string, string>();
            data["snapname"] = "normal";
            data["vmstate"] = "true";
            data["description"] = $"\"AutoGenerated snapshot at {datum} by \"";
            if (ExecuteCommand(ePVEAction.create, $"/nodes/fr1pve/qemu/{vmid}/snapshot", data) == null)
                return false;
            datum = DateTime.UtcNow; Console.WriteLine($"[{datum}] New snapshot for {vmid} created with name {name}!");
            return true;
        }
        //-------------------------------------------------------------------------------------------------------------------------------
        ...
        private static void RedisReader(RedisChannel channel, RedisValue value)
        {
            ...
            {
                DateTime datum = DateTime.UtcNow;
                Console.WriteLine($"[{datum}] Received request on channel {channel} with value {value}");
                ...
                if(res != null && res.auth != null)
                {
                    ...
                    if(CheckAuth(res.master_id,res.auth))
                    {
                        ...
                        {
                            Console.WriteLine($"[{datum}]Invalid operation sent by {returnResult.master_id}...");
                            returnResult.status = false;
                        }
                        //tasks end
                        ...
                        datum = DateTime.UtcNow; Console.WriteLine($"[{datum}] Wrote the response at proxmox:{res.master_id}!");
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"[{datum}] User with wrong credentials tried to send a command ({res.master_id}:{res.auth})!");
                    }
                }
                else
                {
